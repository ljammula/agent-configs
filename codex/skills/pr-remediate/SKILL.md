...
description: >
  ...
  commits onto origin/main and ends in a force push. User-triggered only — never
  run as part of an automatic completion check.
---

# PR Remediate — Rebase recovery (branch partially squash-merged)

This runbook **mutates external state** (ends in `git push --force`). Only run it
