...
description: >
  ...
  Trigger on: "bump this to 0.1.2", "trigger t0.1.1-2", "cut a TestFlight build",
  "what is new between 0.1.1 and 0.1.2", "give me summary for App Store review
  submission", "what is in this release for app store submission", "why not use
  0.1.1", or any pasted `ITMS-#####` error from App Store Connect.
...
```bash
cd ~/code/personal-assistant
~/.claude/skills/testflight-cut/scripts/check-version-sync.sh 0.1.5
```

Six checks, one `PASS`/`FAIL` line each, exit non-zero if any failed:

1. `frontend/pubspec.yaml` `version:` equals the version being tagged
2. `v<version>` is free (local and origin)
3. `t<version>` is free (local and origin)
4. Newer than the last `v` tag
5. Newer than the last `t` tag, and not a reuse of its train
6. Worktree clean — the tag must point at verified code
...
✓ Version sync: 6/6 PASS — pubspec 0.1.5, v0.1.5 and t0.1.5 free, worktree clean at abc1234
...
| `Bash` + `~/.claude/skills/release/scripts/next-version.sh` | Phase 1 — semver suggestion |
