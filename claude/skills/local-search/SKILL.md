...
```bash
curl -sf --max-time 2 http://127.0.0.1:8888/ >/dev/null && echo present
```

If absent, this skill doesn't apply on this machine (or the stack isn't
running right now) -- fall back to WebSearch.
...
unreachable or returns zero results -- in either case, fall back to
WebSearch rather than reporting failure to the user.
