...
| `context-budget-awareness` (not a built-in extension; potential new one) | Not built, real evidence for the gap | `local-model-bench`'s 2026-08-07 Go/Flutter matrix: `pi-local` lost `group-invite-atomic-rotation` to a hard 400 `context_length_budget_exceeded` from the inference route once session prompt tokens (55,339) plus Pi's fixed `max_tokens=16384` request exceeded the route's budget (49152, under its 65536 `max_kv_size`) — reproduced twice, byte-identical trace at temperature 0. Two compounding causes, both on Pi's side: (1) the harness's own system-prompt/`AGENTS.md`/skills/extensions overhead costs ~7,036 prompt tokens on the very first request, before any task content, a fixed tax `terminus-2` (the lean reference agent used as control) doesn't pay; (2) Pi requests a fixed `max_tokens=16384` on every call regardless of already-consumed context, and has no observed handling for this rejection — no shrink-and-retry, no fresh-session fallback (which is what the route's own error message names as the fix). It surfaces as an empty assistant turn and the session just ends. Not yet built as an extension; candidate design: catch `context_length_budget_exceeded`, shrink the requested `max_tokens` and retry once, and if that still doesn't fit, start a fresh session carrying forward a summary rather than full history. |
...
- Build a context-budget-awareness extension (see its new table entry
  above): catch `context_length_budget_exceeded` from the inference route,
  shrink `max_tokens` and retry once, fall back to a fresh session with a
  summary if still too large. Real, reproduced field evidence
  (`local-model-bench`'s 2026-08-07 Go/Flutter matrix) that this is a live
