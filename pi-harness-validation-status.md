...
- Re-run `group-invite-atomic-rotation` (`pi-local`) now that
  `ai-stack-local.ts`'s `contextWindow` is fixed, to get a real before/after
  confirmation that auto-compaction actually recovers this task rather than
  just moving the failure point later. The fix is grounded in Pi's
  documented trigger formula and a corrected budget number, but hasn't
  itself been live-tested against the original failing task yet.
- Once confirmed, re-verify `terminus-2`'s own harness overhead vs.
  `pi-local`'s (~7,036-token baseline tax, see the table entry above) isn't
  hiding a similar contextWindow mismatch on the control side, since that
