...
| `codebase-memory-mcp` 0.9.0 | Default-disabled, trial-only; not globally wired | Disposable Go repo indexed successfully (10,658 nodes/48,988 edges). In one lightweight paired architecture question, memory and ordinary-tools answers both scored 4/4 deterministically and 4/4 from blinded Gemma 4 reviews. Memory saved 2.91% uncached-input-plus-output tokens but took 0.94% longer, 50% more turns, and 41.18% more tool calls; 21/24 calls still used ordinary repo tools. Vendor 99.2% token-reduction claim not confirmed. `n=1`; original 3-5 task/no-tool battery not run. |
